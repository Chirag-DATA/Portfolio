...
// TYPED ROLES
const roles = ['AI & ML Engineer', 'Data Analyst', 'Cloud Enthusiast', 'Power BI Developer', 'Python Developer'];
...
function typeLoop() {
  ...
  if (!deleting && charIdx > current.length) { deleting = true; setTimeout(typeLoop, 1400); return; }
...
function drawParticles() {
  ...
  const color = isDarkNow ? '0,212,255' : '0,100,200';
...
for (let i = 0; i < 26*7; i++) {
  ...
  cell.className = 'contrib-cell c' + weights[Math.floor(Math.random() * weights.length)];
